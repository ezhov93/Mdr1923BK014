...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), mdrLoader(new MdrLoader) {
  ui->setupUi(this);
  ...
  connect(mdrLoader, SIGNAL(statusChanged(QString)), ui->sL,
          SLOT(setText(QString)));
  connect(mdrLoader, SIGNAL(textChanged(QString)), ui->t,
          SLOT(appendPlainText(QString)));
  connect(mdrLoader, SIGNAL(dumpFilenameChanged(QString)), ui->dumpfnLE,
          SLOT(setText(QString)));
  connect(mdrLoader, SIGNAL(stopButtonSetEnable(bool)), ui->touchMCUPB,
  ...
  // ByteArray::test();
  /*  WinMerge говорит "Выбранные файлы не имеют отличий" *YAHOO*
  Hex hex;
  qDebug()<<hex.loadFromFile(ui->fwfnLE->text());
  qDebug()<<Hex::saveToFile(ui->fwfnLE->text().append("_static.hex"),
  hex.data(), hex.baseAdr(), hex.startAdr());
  qDebug()<<hex.saveToFile(ui->fwfnLE->text().append("_member.hex")); */
}
...
void MainWindow::on_touchMCUPB_clicked() {
  ...
  if (ui->mcuVC1RB->isChecked()) mcuType = Mcu::VC1;
...
void MainWindow::on_openFilePB_clicked() {
  QString fn = QFileDialog::getOpenFileName(
      this, tr("Файл прошивки"), ui->fwfnLE->text(), Hex::supportedFormats());
  ui->fwfnLE->setText(QDir::toNativeSeparators(fn));
}

void MainWindow::on_dumpfnPB_clicked() {
  QString fn = QFileDialog::getSaveFileName(
      this, tr("Файл дампа"), ui->dumpfnLE->text(), Hex::supportedFormats());
  ui->dumpfnLE->setText(QDir::toNativeSeparators(fn));
}

void MainWindow::on_flasherfnPB_clicked() {
  QString fn = QFileDialog::getOpenFileName(
      this, tr("Файл флешера"), ui->dumpfnLE->text(), Hex::supportedFormats());
...
void MainWindow::on_about_triggered() {
  QString text(
      "Эта версия программы во многом аналогична программам от vasili "
      "(forum.milandr.ru), ");
  ...
  text.append(
      "Доработать загрузчик для возможности частичного стирания основной "
      "памяти, "
      "работы с информационной памятью.\n");
  text.append(
      "Предложения и замечания принимаются и приветствуются в личку на форуме "
      "forum.milandr.ru пользователю prostoRoman или на "
      "prostoRoman@gmail.com\n");
  text.append(
      "\nЛицензия, исходники позже. Информация здесь может быть уже не "
      "актуальна ;)\n");
  text.append("03.08.2017 Роман");
  QMessageBox mb(QMessageBox::Information,
                 tr("О программе ").append(windowTitle()), text,
                 QMessageBox::Ok, this);
  mb.exec();
}
...
void MainWindow::writeSettings() {
  ...
  settings.setValue("vc1", ui->mcuVC1RB->isChecked());
...
void MainWindow::readSettings() {
  ...
  ui->fwfnLE->setText(
      settings.value("fwfn", qApp->applicationDirPath()).toString());
  ...
  ui->mcuVC1RB->setChecked(settings.value("vc1", false).toBool());
  ui->loadCB->setChecked(
      settings.value("load", false).toBool());  // on_loadCB_clicked
  ui->dumpfnLE->setEnabled(settings.value("load", false).toBool());
  ui->dumpfnPB->setEnabled(settings.value("load", false).toBool());
  ...
  // qDebug()<<"sett find
  // ="<<ui->portBaudrateCB->findText(settings.value("baudrate",
  // 115200).toString());
