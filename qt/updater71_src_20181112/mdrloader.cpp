...
MdrLoader::MdrLoader(QObject *parent)
    : QObject(parent),
      flasher(new Hex),
      firmware(new Hex),
      work(NoWork),
      stop(false),
      mcuType(Mcu::Undef)  //, dumpFile(0)
{
...
bool MdrLoader::checkAndLoadFiles() {
  ...
  if (work & Program || work & Verify) {
    ...
    if (Hex::EOF_DETECTED != errCode) {
      emit textChanged(
          tr("Невозможно открыть файл прошивки, код ошибки %1, строка %2.")
              .arg(errCode)
    ...
    if (firmware->baseAdr() == 0xFFFFFFFF) {
      firmware->setData(firmware->data(), Mcu::mem[mcuType].flashAdr,
      ...
      if ((firmware->baseAdr() < Mcu::mem[mcuType].flashAdr) ||
        ...
        emit textChanged(tr("Загружаемая программа выходит за пределы "
                            "диапазона flash-памяти! "
                            "Начинается 0x%1, заканчивается 0x%2")
                             .arg(firmware->baseAdr(), 8, 16, QChar('0'))
                             .arg(firmware->baseAdr() + firmware->sizeData(), 8,
                                  16, QChar('0')));
        return false;
      }
    ...
    qDebug() << "  base =" << QString::number(firmware->baseAdr(), 16)
             << " start =" << QString::number(firmware->startAdr(), 16)
             << "  size =" << QString::number(firmware->sizeData(), 10);
  }
  if (work & Load) {
  }
  if (flasherfn.isEmpty()) {
    switch (mcuType) {
      ...
      case Mcu::BE4:
        ...
        break;
      case Mcu::VC1:
        flasherfn = ":/flashers/1901VC1_BOOT_UART.hex";
        break;
      default:
        break;
    }
  ...
  if ((flasher->baseAdr() < Mcu::mem[mcuType].xramAdr) ||
    ...
    emit textChanged(
        tr("Загружаемая программа выходит за пределы диапазона XRAM-памяти! "
           "Начинается 0x%1, заканчивается 0x%2")
...
QByteArray MdrLoader::portWR(const QByteArray &send, int bytesCount,
                             int timeout) {
  QTimer timer(parent());
  QByteArray ba;
  ...
  if (!respond.contains(ba)) {
    ...
    for (int chank = 0; chank < chankCount; chank++) {
      ...
      if (!respond.contains(flasher->data().mid(chank * 8, 8))) {
        emit textChanged(
            "Не прошла команда верификации флешера (верификация)!");
        return false;
      }
    }
...
bool MdrLoader::dump() {
  ...
  if (dumpfn.isEmpty()) {
    emit textChanged(
        tr("Задайте имя файла для сохранения дампа прошивки "
           "или снимите флажок Load."));
    return false;
  }

  if (!(dumpfn.endsWith(".bin", Qt::CaseInsensitive) ||
        dumpfn.endsWith(".hex", Qt::CaseInsensitive)))
    dumpfn.append((".hex"));
  emit dumpFilenameChanged(dumpfn);
  /** Изначально рассчёт был проверять на возможность программы открыть файл на
  запись
   * ПЕРЕД скачиванием прошивки,
   * но поскольку невозможность открыть файл - это ошибка (т.е. не штатная
  ситуация),
   * то не грех разок считать прошику в холостую...
  dumpFile = new QFile;
  dumpFile->setFileName(dumpfn);
  if( !dumpFile->open(QIODevice::WriteOnly) ) {
      emit textChanged(tr("Не возможно открыть Файл для сохранения дампа
  прошивки: ") .append(dumpFile->errorString())); delete dumpFile; dumpFile = 0;
      return false;
  } */
  ...
  /*    if(dumpFile) {
          ...
          Hex::saveToFile(dumpFile->fileName().append(".hex"), dump,
                          Mcu::mem[mcuType].flashAdr,
     Mcu::mem[mcuType].flashAdr); dumpFile->close(); delete dumpFile; dumpFile =
     0; } else { emit textChanged(tr("Не был создан Файл для сохранения дампа
     прошивки!")); return false;
      } */
...
bool MdrLoader::program() {
  ...
  emit statusChanged(
      tr("Программирование %1 байт flash-памяти...").arg(firmware->sizeData()));
  ...
  for (int chank = 0; chank < chankCount; chank++) {
    ...
    if ((respond.size() < 1) || ((unsigned char)respond.at(0) != ks)) {
      emit textChanged(tr("Проблема при передаче данных на %1%.")
                           .arg((double)chank / chankCount * 100.0));
      qDebug() << "r.size =" << respond.size() << "r.data =" << respond.toHex()
...
bool MdrLoader::verify() {
  ...
  emit statusChanged(
      tr("Верификация %1 байт flash-памяти...").arg(firmware->sizeData()));
  ...
  for (int chank = 0; chank < chankCount; chank++) {
    ...
    if (respond != ba) {
      emit textChanged(tr("Ошибка верификации данных на %1%.")
                           .arg((double)chank / chankCount * 100.0));
      emit textChanged(
          tr("По адресу 0x%1 считано %2, должно быть %3.")
