...
void gdb_set_register(void *hw_regs) { regs = hw_regs; }
void gdb_get_register(unsigned long *gdb_regs) {
  int regno;
  /* Initialize all to zero. */
  for (regno = 0; regno < GDB_MAX_REGS; regno++) gdb_regs[regno] = 0;
...
int gdb_arch_handle_exception(char *remcom_in_buffer, char *remcom_out_buffer) {
  ...
  switch (remcom_in_buffer[0]) {
    ...
    case 's':
      ptr = &remcom_in_buffer[1];
      if (gdb_hex2long(&ptr, &addr)) regs->pc = addr;
      ...
      if ((ins & 0x02000000) == 0) {
        ...
        if (ins_will_execute(ins)) {
          ...
          if ((ins & 0xFE000000) == 0xFA000000) {
            if ((ins & 0x01000000) == 0x01000000) new_pc |= 2;
