...
void ClockConfig(void) {
  ...
  //  CLK_CNTR->PLL0_CLK = (2 << 29)|(8 << 8)|(0 << 0);   // PLL_FREQ = (10 MHz
  //  * 8) / 1 = 80 MHz CLK_CNTR->PLL0_CLK |= (1 << 27);    //PLL ON
  //  while((CLK_CNTR->PLL0_STAT & (1 << 20)) == 0){} //wait while PLL0 not
  //  ready CLK_CNTR->MAX_CLK = 8;  //MAX_CLOK <-- PLL0 cpu freq = 80 MHz
}
...
void UartConfig(void) {
  CLK_CNTR->UART0_CLK =
      (2 << 28) | (1 << 16);      // Frequency UART0 -> 8 MHz (HSE1)
  ...
  MDR_UART0->CR =
      (1 << 9) | (1 << 8) | (1 << 0);  // Tx enable, Rx enable, Uart0 enable.
}
