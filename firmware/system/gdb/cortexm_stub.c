...
int gdb_set_hw_break(unsigned long addr, int len, enum gdb_bptype bptype) {
  ...
  if (bptype == BP_HARDWARE_BREAKPOINT) {
    for (i = 0; i < HBP_NUM; i++)
      if (!breakinfo[i].enabled) break;
    if (i == HBP_NUM) return -1;
    breakinfo[i].addr = addr;
    breakinfo[i].enabled = 1;
  } else if (bptype == BP_WRITE_WATCHPOINT) {
    for (i = 0; i < HWP_NUM; i++)
      if (!watchinfo[i].enabled) break;
    if (i == HWP_NUM) return -1;
...
int gdb_remove_hw_break(unsigned long addr, int len, enum gdb_bptype bptype) {
  ...
  if (bptype == BP_HARDWARE_BREAKPOINT) {
    for (i = 0; i < HBP_NUM; i++)
      if (breakinfo[i].addr == addr && breakinfo[i].enabled) break;
    if (i == HBP_NUM) return -1;
...
void gdb_arch_late() { gdb_remove_all_hw_break(); }
...
void gdb_arch_exit() {
  ...
  for (i = 0; i < HWP_NUM; i++) {
    if (watchinfo[i].enabled) {
      ...
      *base = num -
              1;  // DWT matching is performed as:(ADDR & (~0 << MASK)) == COMP
